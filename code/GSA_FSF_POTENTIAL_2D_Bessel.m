...
% u1=A*sin(X.*Y./largo^2).*(exp(-(X).^2./largo^2-(Y).^2./largo^2));
...
bessel_order = 3;
% POT =  height_potential .* exp(-(0.35*r).^2).* besselj(bessel_order,period_potential*r); 
% very interesting concentric rings for above

 % p = 2; % Radial mode number
% l = 0; % Azimuthal mode number

% Azimuthal angle
% th = atan2(Y, X);

% POT = height_potential.* exp(-0.35.*r.^2 ) .* laguerreL(p, abs(l), (period_potential*r).^2) .* exp(1i * l * th);
...
v = VideoWriter(plus(string(bessel_order),plus("Bessel_",nm)),"MPEG-4");

open(v)

f2 = figure;

v2 = VideoWriter(plus(string(bessel_order),plus("Bessel_cont_",nm)),"MPEG-4");
